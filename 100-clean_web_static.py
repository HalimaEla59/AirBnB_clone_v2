#!/usr/bin/python3
"""Fabric scripts that generates a .tgz archive from web_static"""
import os
...
def do_clean(number=0):
    ...
    local('cd versions ; ls -t | tail -n +{} | xargs rm -rf'.format(nm))
    path = '/data/web_static/releases'
    run('cd {} ; ls -t | tail -n +{} | xargs rm -rf'.format(path, nm))
